## javascript编程指南
来自(https://github.com/airbnb/javascriptd),主要是ECMAScript6一些规范指导。
...
blocks(块)、Comments(注释)、Whitespace(注释)、Commas(逗号)、Semicolons(分号)、Naming Conventions（命名惯例）这些就不详细说明，可以通过eslint配置规则，需要说明的是，比如注释规则和命名规则等都应该遵循团队规范。
